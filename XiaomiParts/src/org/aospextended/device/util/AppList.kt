...
class AppList: Fragment(R.layout.app_list_layout) {
    ...
    private fun addApp(packageName: String) {
        ...
        Settings.System.putString(context?.contentResolver,
                getKey(), apps.joinToString(separator = ","))
    }

    private fun removeApp(packageName: String) {
        if (apps.contains(packageName)) apps.remove(packageName);
        Settings.System.putString(context?.contentResolver,
                getKey(), apps.joinToString(separator = ","))
    }

    private fun getKey(): String {
        return "game_app_list"
    }
    ...
    private fun refreshList() {
        var list = packageList.filter {
            when (category) {
                ...
                CATEGORY_USER_ONLY -> it.applicationInfo?.isSystemApp() == false
                else -> it.applicationInfo != null
            }
        }.filter {
